...
- **Cloud coding provider updated:** Codex is now the preferred cloud coding environment throughout the plan, including the cloud/local routing, privacy boundary, implementation phases, decisions, and target end state.
...
Everything else from v2 (data classification, no direct bank/Gov.br authentication, Qwen-only local strategy, Codex for cloud coding, MCP as tool abstraction, agent gateway design) remains unchanged and is carried forward below.
...
- Codex for selected cloud-based coding tasks.
...
The architecture is intentionally model-agnostic so that Qwen, Codex, Ollama, MCP components, or other technologies can be replaced later without redesigning the whole platform.
...
Codex is the preferred cloud coding environment for tasks where cloud processing is acceptable:
...
**Privacy boundary:** Codex must never receive RED-classified data (section 8).
...
Codex is preferred when cloud processing is allowed and maximum coding capability is useful. Local Qwen/Qwen3-Coder is preferred when code or repository data is sensitive and should remain local.
...
- Codex confirmed for cloud coding.
...
### Phase 9 — Coding
Codex integration, local Qwen coding, repository classification, cloud/local routing, Git integration.
...
| Codex for cloud coding | **DECIDED** |
...
10. Codex is the preferred cloud coding environment.
...
Non-sensitive repository → Codex
```
